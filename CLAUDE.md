...
assets/glyphs.js       small frosted 3D objects: any element with data-glyph="name" gets its own canvas (one shared offscreen renderer)
...
- 3D is part of the brand (founder decision, 2026-09-24): hero background field, coverage bars in the brief, research engine by the demos, cohort surface in alt data, enclave in security, plus small frosted "glyphs" (see `assets/glyphs.js`). A glyph is only added where the object pictures the item next to it (strategy packs: long/short, balanced pair, capital stack, broken stack, merging companies, signal surface, globe; alt-data checks; the security enclave; the logo). Never add one just to fill space. Keep motion slow and subtle. New 3D should tie to something real where possible. All scenes pause off-screen, respect `prefers-reduced-motion`, and the page works without WebGL.

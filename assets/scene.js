// Deskside: 3D scenes. The page reads fine without WebGL or JS
// (main.js draws a flat version of the hero coverage bars).
...
/* ---------- Hero background: a slow-moving field of data points ---------- */
...
/* ---------- Demos: research engine (sources flow in, flags flow out) ---------- */
...
/* ---------- Security: sealed enclave (data moves inside, never leaves) ---------- */
...
if (webglOK()) {
  ...
  const scenes = {
    "hero-3d": heroTerrain,
    "engine-3d": researchEngine,
    "enclave-3d": enclave,
    "cohort-3d": cohortSurface,
  };
  for (const [id, fn] of Object.entries(scenes)) {
    const el = document.getElementById(id);
    if (el) fn(el);
  }
  const cov = document.getElementById("coverage-3d");
  if (cov && window.desksideCoverage) coverageBars(cov, window.desksideCoverage);
}
